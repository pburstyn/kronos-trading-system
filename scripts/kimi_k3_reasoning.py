...
KIMI_K3_SYSTEM_PROMPT = f"""You are Kimi K3, an expert trading analyst assistant. Today is {today}. You receive a signal from a quantitative model and provide a short, honest reasoning analysis.
...
def ask_kimi_k3(signal):
    ...
    payload = {
        ...
        "messages": [
            {"role": "system", "content": KIMI_K3_SYSTEM_PROMPT},
    ...
    if not choices:
        print(f"WARNING: Kimi K3 response had no choices: {result}")
        return None

    content = choices[0].get("message", {}).get("content")
    if not content:
        print(f"WARNING: Kimi K3 response had no message content: {result}")
        return None
...
def log_reasoning(signal, reasoning):
    if not reasoning:
        reasoning = "Kimi K3 returned no reasoning"

    os.makedirs(os.path.dirname(KIMI_K3_REASONING_LOG), exist_ok=True)
    file_exists = os.path.isfile(KIMI_K3_REASONING_LOG)
    with open(KIMI_K3_REASONING_LOG, "a", newline="") as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow([
                "timestamp", "ticker", "direction",
                "confidence_pct", "last_close", "kimi_k3_reasoning"
            ])
        writer.writerow([
            signal['timestamp'],
            signal['ticker'],
    ...
    print(reasoning)
    print("----------------------")
    print(f"Logged to: {KIMI_K3_REASONING_LOG}\n")
...
def run():
    ...
    if direction not in ("UP", "DOWN"):
        print(f"Signal is {direction} -- not actionable. Skipping Kimi K3 analysis.")
        return

    print(f"Signal: {signal['ticker']} -> {signal['direction']} ({signal['confidence_pct']}% confidence)")
    print("Asking Kimi K3 for reasoning...\n")

    reasoning = ask_kimi_k3(signal)
